...
Use one focused source commit containing only `selectContext.mjs` and `tests/context-selector.test.mjs`. Later commits may add evidence only. `decision.md` must compare planned and actual context, explain any expansion, and state the correctness and cost trade-off.
