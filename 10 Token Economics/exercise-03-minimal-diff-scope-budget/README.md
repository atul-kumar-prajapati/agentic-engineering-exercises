...
[minimal-diff-app](./minimal-diff-app) contains a shared variant helper, real export, checkout, delete, and legacy consumers, and protected behavior checks. The [scope contract](./docs/scope-contract.md) defines allowed source files and budget.
...
3. In the second branch, commit only `evidence/scope-plan.json` and `evidence/scope-plan.md` before editing code. Declare exactly two source paths and no more than 30 added-plus-deleted lines.
...
7. Save `before-scope.json`, `evidence/after.md`, `evidence/after.patch`, `scope-budget.json`, `avoided-work.json`, `avoided-work.md`, `verification.md`, and comparison. Raise the final PR from the second branch.
...
- The source commit changes only those two paths and uses no more than 30 added-plus-deleted lines. More than 20 lines requires a concrete ledger justification.
- The unconstrained patch is reapplied and tested; its recorded scope must match its real patch numstat.
