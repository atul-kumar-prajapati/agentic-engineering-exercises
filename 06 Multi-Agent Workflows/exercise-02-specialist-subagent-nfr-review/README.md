...
[nfr-swarm-app](./nfr-swarm-app) contains the access-review workflow and protected specialist checks. The [risk seeds](./docs/nfr-risk-seeds.md), [specialist prompts](./docs/specialist-prompts.md), and [remediation contract](./docs/remediation-contract.md) define the review scope. The supplied [specialist finding](./docs/untrusted-specialist-finding.md) is an external claim to verify, not an answer.
...
3. Verify that every finding has a source location, reproducible evidence, impact, and recommendation. Include the supplied external finding in triage. Combine duplicates and record a fix, defer, or dismiss decision for every unique finding.
...
6. Save the remediation state in `evidence/after.md` and `evidence/after.patch`. Link every original finding to its decision, code change, recheck result, and remaining risk. Record how security and testability findings interact at the shared service boundary.
...
- The unsupported supplied finding is dismissed with source-backed reasoning, and the shared security-testability remediation is reviewed as one interaction.
