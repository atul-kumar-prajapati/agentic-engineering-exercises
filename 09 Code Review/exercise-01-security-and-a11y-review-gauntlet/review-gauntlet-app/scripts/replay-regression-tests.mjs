...
const testRelative = "tests/review-regressions.test.ts";
...
const temporary = fs.mkdtempSync(path.join(os.tmpdir(), "review-regression-replay-"));
...
try {
  ...
  const afterOutput = path.join(os.tmpdir(), `review-after-${process.pid}-${Date.now()}.json`);
  const after = run(appRoot, afterOutput);
  fs.rmSync(afterOutput, { force: true });
  if (before.result.status === 0) throw new Error("learner regression tests unexpectedly pass on the vulnerable review head");
  if (before.report.numFailedTestSuites > 0 && before.assertions.length === 0) throw new Error("vulnerable-head run failed during collection instead of executing regression assertions");
  for (const id of findingIds) if (!assertionFor(before.assertions, id, "failed")) throw new Error(`${id} does not have a named regression assertion that fails on the vulnerable head`);
